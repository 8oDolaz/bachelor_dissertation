"""
BC-Transformer training script with image observations (no object pose).
...
Usage:
    python resnet_transformer.py --dataset /path/to/dataset.hdf5 --output /path/to/output_dir
    python resnet_transformer.py --debug
"""
import argparse
import gc
import os
...
def freeze_backbone_except_last(model):
    ...
    The SpatialSoftmax pooling, FC projection, Transformer, and policy head
...
def set_hyperparameters(config):
    """
    Sets hyperparameters for BC-Transformer training with image observations.
...
def get_config(dataset_path=None, output_dir=None, debug=False,
    ...
    ### Experiment Config ###
    config.experiment.name = f"resnet_transformer_{config.train.num_epochs}"
